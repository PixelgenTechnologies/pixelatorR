...
- [`MoleculeRankPlot()`](MoleculeRankPlot.md) : Molecule Rank Plot
...
- [`layout_with_spectral()`](layout_with_spectral.md) : Spectral Layout
  via Graph Laplacian
- [`layout_with_weighted_pmds()`](layout_with_weighted_pmds.md) :
  Weighted PMDS
- [`layout_with_coarsened_pmds()`](layout_with_coarsened_pmds.md) :
  Coarsened pMDS
...
- [`ComputeProximityScores()`](ComputeProximityScores.md) : Compute
  proximity scores
...
- [`downsample_to_parquet()`](downsample_to_parquet.md) : Downsample
  edgelist and export to parquet files
- [`lcc_curve()`](lcc_curve.md) **\[experimental\]** : Compute LCC curve
  for downsampled edgelists
- [`lcc_sizes()`](lcc_sizes.md) : Compute LCC sizes from downsampled
  edgelists
...
- [`local_proximity()`](local_proximity.md) : Compute local proximity
  scores

### Cell segmentation

- [`segment_cell()`](segment_cell.md) : cell:cell conjugate segmentation
- [`cc_protein_weights()`](cc_protein_weights.md) : Derive protein
  weights for two cell populations
- [`distance_from_node_set()`](distance_from_node_set.md) : Compute
  distance from a set of seed nodes using fast breadth-first search.
- [`partition_counts()`](partition_counts.md) : Compute partition counts
- [`spatial_smoothing()`](spatial_smoothing.md) : Perform spatial
  smoothing of a count matrix using the adjacency matrix of the graph
- [`fetch_node_neighborhoods()`](fetch_node_neighborhoods.md) : Fetch
  node neighborhoods from pxl file(s) attached to a Seurat object
...
- [`Pixelgen_cell_palette`](Pixelgen_cell_palette.md) : Pixelgen cell
  type colors
- [`heuristic_illumination()`](heuristic_illumination.md) : Compute
  heuristic illumination for 3D layouts
