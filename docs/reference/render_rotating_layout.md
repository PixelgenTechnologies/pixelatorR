...
- use_illumination:
  ...
  A logical value indicating whether to modulate `node_val` colors with
  a heuristic illumination mask. Default is `FALSE`.

- illumination_ambient:

  A numeric value between 0 and 1 specifying the minimum brightness
  floor when blending illumination. Default is `0.3`.

- illumination_sat_boost:

  A non-negative numeric value controlling saturation compensation in
  shadowed regions when using HSV blending. Ignored when
  `illumination_shadow_colors` is set. Increase this to increase
  saturation in darker points. Default is `0.6`.

- illumination_shadow_colors:

  `NULL` or a vector of valid colors. When `NULL` (default),
  illumination is blended in HSV space. When set, each point's
  illumination is mapped to this palette instead and linearly
  interpolated with the base `node_val` color.
...
When `use_illumination = TRUE`, node colors are derived from `node_val`
and then modulated by a geometry-based illumination mask computed with
[`heuristic_illumination`](heuristic_illumination.md). This adds depth
cues on top of the marker color encoding. By default, shadows are
blended in HSV space using `illumination_ambient` as the minimum
brightness floor (not the ambient-occlusion weight in
`heuristic_illumination`) and `illumination_sat_boost` for saturation
compensation in shadowed regions. When `illumination_shadow_colors` is
set, shadows are instead blended toward a second color gradient mapped
from the illumination mask; in that mode `illumination_sat_boost` is
ignored. `PixelgenGradient(n, "NaturalBlue")` works well as a shadow
