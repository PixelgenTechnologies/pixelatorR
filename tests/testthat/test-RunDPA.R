...
test_that("RunDPA fails with invalid input", {
  ...
  expect_error(
    dpa_markers <- RunDPA(polarization_table_merged, contrast_column = "sample", target = "Sample1"),
    'argument "reference" is missing, with no default'
  )
  expect_error(
    dpa_markers <- RunDPA(polarization_table_merged, contrast_column = "Invalid", target = "Sample1", reference = "Sample2"),
    "'contrast_column' must be a valid column name"
  )
  expect_error(
    dpa_markers <- RunDPA(polarization_table_merged, contrast_column = "sample", target = "Invalid", reference = "Sample2"),
    "'target' must be present in 'contrast_column' column"
  )
  expect_error(
    dpa_markers <- RunDPA(polarization_table_merged, contrast_column = "sample", target = "Sample1", reference = "Invalid"),
