...
test_that("CellCountPlot works for Seurat objects", {
  expect_no_error({cellcount_plot <- CellCountPlot(seur_obj, color_by = "leiden", as_frequency = TRUE)})
  ...
  expect_no_error({cellcount_plot <- CellCountPlot(seur_obj, group_by = "leiden", color_by = "tau_type")})
  expect_no_error({cellcount_plot <- CellCountPlot(seur_obj, group_by = "leiden", color_by = "sample", as_frequency = TRUE, stack = TRUE)})
})
