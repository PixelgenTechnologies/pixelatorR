for (assay_version in c("v3", "v5")) {
  ...
  test_that("Plot3DGraph works as expected", {
    expect_no_error({
      layout_plot <- Plot3DGraph(seur_obj, cell_id = colnames(seur_obj)[1], layout_method = "pmds_3d", marker = "CD14")
    })
    expect_no_error({
      layout_plot <- Plot3DGraph(seur_obj, cell_id = colnames(seur_obj)[1], layout_method = "pmds_3d", marker = "CD14")
    })
    ...
    expect_no_error({
      layout_plot <- Plot3DGraph(seur_obj, cell_id = colnames(seur_obj)[1], layout_method = "pmds_3d", show_Bnodes = TRUE, marker = "CD14")
    })

    # Test with project active
    expect_no_error({
      layout_plot <- Plot3DGraph(seur_obj, cell_id = colnames(seur_obj)[1], layout_method = "pmds_3d", project = TRUE, marker = "CD14")
    })
  })
