...
test_that("CalculateDispersion works as expected", {
  ...
  ## Edge-case tests for potential division-by-zero
  zero_row_mat <- matrix(
    c(
      0, 0, 0,
      1, 2, 3
  ...
  expect_no_error(disp_zero_row_1 <- CalculateDispersion(zero_row_mat, margin = 1, method = "gini"))
  expect_equal(
    disp_zero_row_1,
    c(0, 0.222222222222222)
  )
  expect_no_error(disp_zero_row_1 <- CalculateDispersion(zero_row_mat, margin = 1, method = "tau"))
  expect_equal(
    disp_zero_row_1,
    c(0, 0.5)
  )
