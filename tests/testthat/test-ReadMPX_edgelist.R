test_that("ReadMPX_arrow_edgelist works as expected", {
  ...
  expect_equal(names(el), c("upia","upib","umi","marker","sequence","count",
                            "umi_unique_count", "upi_unique_count","component","sample"))
  expect_no_error({msg <- capture_messages({el <- ReadMPX_arrow_edgelist(system.file("extdata/PBMC_10_cells", "Sample01_test.pxl", package = "pixelatorR"),
                         overwrite = TRUE)})})
})
...
test_that("ReadMPX_arrow_edgelist fails when invalid input is provided", {
  ...
  expect_error({suppressWarnings({el <- ReadMPX_arrow_edgelist(system.file("extdata/PBMC_10_cells", "Sample01_test.pxl", package = "pixelatorR"),
